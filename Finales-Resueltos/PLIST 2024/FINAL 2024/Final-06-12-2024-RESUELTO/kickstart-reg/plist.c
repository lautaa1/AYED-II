...
static bool invrep(plist l) {
    ...
    if (!plist_is_empty(l)){
        if (l->atras!=NULL){
            struct s_node *iter1 = l->atras;
            while (iter1 != NULL){
                if (!t_elem_less(iter1->elem,l->pivot)){
                    valid = false;
                }
                iter1 = iter1->next; 
            }
        ...
        if (l->adelante!=NULL){
            struct s_node *iter2 = l->adelante;
            while (iter2 != NULL){
                if (!t_elem_greater(iter2->elem,l->pivot)){
                    valid = false;
                }
                iter2 = iter2->next; 
            }
...
plist plist_add(plist l, t_elem e) {
    ...
    if (t_elem_less(e, l->pivot)){
        struct s_node *ult_pos = l->atras;
        if (ult_pos == NULL) {
            l->atras = new_elem;
        } else {
            while (ult_pos->next != NULL){
                ult_pos = ult_pos->next;
            }
            ult_pos->next = new_elem;
        }
